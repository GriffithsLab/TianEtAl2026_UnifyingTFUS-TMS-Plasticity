...
% params.dth = 0.2e-6; % 25 Dec 31
% params.pth = 0.3e-6; % 25 Dec 31

% params.dth = 0.2e-6; % 26 Jan 2
% params.pth = 0.34e-6; % 26 Jan 2
...
% params.ltp = 1e-2; % 25 Dec 29
% params.ltd = 5e-2; % 25 Dec 29
...
params.Duration = 80; %total stim time
...
% % only use the following three lines for "analyzing varying duty cycle with
% % total energy controled by duration (original 80s for duty_cycle=0.1))"
% params.Duration = 80/(duty_cycle/0.1); %total stim time
% T = T - (80-params.Duration);
% nsteps = round(T/deltat);
% 



params.stim.cont.sin_freq_hz      = 500;      % 500Hz, sinusoid frequency inside pulse 
                               % (max=2500 if dt=1e-4: 5 sampling points for one cycle)


r_a_m = 0.3; % 0.3; the ratio "osci amp"/"osci mean"
...
% params.stim.cont.mean             = (sqrt(3/(2+r_a_m^2)))*((sqrt(5)/10)*amp_TMS);   % cTBS(3,5) with pulse_width=0.5ms  
