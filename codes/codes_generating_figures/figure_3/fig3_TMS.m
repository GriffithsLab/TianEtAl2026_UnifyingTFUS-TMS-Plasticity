% duration = 40; cTB_TMS_40s
% duration = 80; cTB_TMS_80s

function [nu_ee, nutil_ee, Ca_ee,pct_change_nu_ee] = fig3_TMS(duration)

    %% --- Grid and time ---
    Nx = 1; Ny = 1;      % single node of Nx=Ny=1
    T = 650 + duration;             % 600; simulation time [s] 
    deltat = 1e-4;       
    ...
    params.dth = 0.2e-6; %26 Jan 2
    params.pth = 0.336e-6; %26 Jan 2
    ...
    % Kevin's TMS-amp scaling (params.amp):
    % (3,5):105; (2,5):136.5; (3,3):147; (3,7):81; (5,5):75.6
    
    params.TMS_scale_x = 1;
    
    params.nu_x = 0.008222; %Kevin
    
    % t_3_5_scale = 105; t_2_5_scale = 136.5; t_3_3_scale = 147;t_3_7_scale = 81; t_5_5_scale = 75.6;
    % 
    % my_tms_amp_scale = 0.29; % 
    
    %params.amp = my_tms_amp_scale*t_3_5_scale; %100;24
    
    %params.amp = 42.63; % Kevin's paper for (3,3)
    %params.amp = 30.45; % Kevin's paper for (3,5)

    %params.amp = 23.49; % Kevin's paper for (3,7)
    ...
    params.onset = 50; params.Duration = duration; params.on = 2; params.off = 0;
    ...
    for step = 1:nsteps
        ...
        % phi_x   = TMS(step-1, params);
        % noise_x = params.noise.enabled * white_stim_new(step-1, params);
    
        % ---- Stimuli (scalars, applied identically to all nodes) ----
        abs_step = (step-1) + nsteps_warm;           % <<< NEW: absolute time index
        phi_x   = TMS(abs_step, params);             % was: TMS(step-1,...)
