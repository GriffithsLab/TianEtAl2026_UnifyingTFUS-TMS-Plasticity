...
function pct_change_nu_ee_vec = fig4e(duty_cycle_vec)

for jj = 1:length(duty_cycle_vec)
    %% --- Grid and time ---
    Nx = 1; Ny = 1;      % single node of Nx=Ny=1
    T = 350 + 8/duty_cycle_vec(jj);             % 600; simulation time [s]
    deltat = 1e-4;
    ...
    params.Duration = 8/duty_cycle_vec(jj); %total stim time
    ...
    params.stim.cont.mean             = 10.72;   % cTBS(3,5) with pulse_width=0.5ms  
