...
% Initialize:
% duration_vec = [20,40,60,80,100,120,140,160,180];
% pct_change_nu_ee_vec = zeros(1,length(duration_vec));

function pct_change_nu_ee_vec = fig4b(duration_vec)

for jj = 1:length(duration_vec)
    %% --- Grid and time ---
    Nx = 1; Ny = 1;      % single node of Nx=Ny=1
    T = 350 + duration_vec(jj);             % 600; simulation time [s]
    deltat = 1e-4;
    ...
    params.Duration = duration_vec(jj); %total stim time
