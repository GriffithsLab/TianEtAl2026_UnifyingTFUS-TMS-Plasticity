...
pct_change_nu_ee_vec_py = np.zeros(len(duty_cycle_vec))

for ii in range(0,len(duty_cycle_vec)):
    ...
    params.Duration = 8/duty_cycle_vec[ii]  # total stimulation time [s]
    
    # ---- Continuous sinusoidal pulse-train config ----
    params.stim.cont = SimpleNamespace()
    params.stim.cont.prf_hz = 5 # 5
    
    duty_cycle = duty_cycle_vec[ii]
